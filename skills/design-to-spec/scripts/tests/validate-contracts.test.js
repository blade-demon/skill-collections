        ...
        endpoints: [
          {
            ...
            method: "GET",
            params: [
              { name: "keyword", type: "string", required: true, notes: "" },
            ],
            response_fields: [
              {
                name: "items",
...
test("state scope_components references missing component", () => {
  ...
  expectFailContains(runValidate(docs), "scope_components references missing component 'nonExistentBtn'");
});
...
test("ui_to_api binding source_ui references missing component", () => {
  ...
  expectFailContains(runValidate(docs), "binding source_ui references missing component 'ghostBtn'");
});
...
test("ui_to_api binding target_api references missing param", () => {
  ...
  expectFailContains(runValidate(docs), "binding target_api references missing API param 'ghostParam'");
});

test("api_to_ui binding source_api references missing field", () => {
  const docs = baseline();
  docs.mapping.mapping.bindings[0].source_api = "ghostField";
  expectFailContains(runValidate(docs), "binding source_api references missing response field 'ghostField'");
});

test("api_to_ui binding target_ui references missing component", () => {
  const docs = baseline();
  docs.mapping.mapping.bindings[0].target_ui = "ghostBtn";
  expectFailContains(runValidate(docs), "binding target_ui references missing component 'ghostBtn'");
});
