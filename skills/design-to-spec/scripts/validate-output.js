...
/**
 * 把当前三份输出文件里的所有 trace anchor 保存为 JSON 基线（4A 通过后调用）。
 * 4B 完成后用 checkNoNewEntities() 与基线对比，确保 LLM 没有新增契约外实体。
 */
function saveBaseline(notesText, dataFetchingText, specText, baselinePath) {
...
/**
 * 与 4A 基线对比，返回 4B 阶段新增的 trace anchor 列表。
...
function checkNoNewEntities(notesText, dataFetchingText, specText, baselinePath) {
  ...
  return [...currentIds].filter((id) => !baselineIds.has(id));
}
  ...
  if (args['no-new-entities']) {
    if (!existsSync(args['no-new-entities'])) {
      ...
      for (const id of added) {
        errors.push(
          `4B introduced entity not in 4A baseline: ${id} — LLM must only write to NARRATIVE sections`,
        );
      }
    }
  }
