...
function generateNotes(ui, api, mapping, capability) {
  ...
  if (events.length > 0) {
    for (const event of events) {
      const eventType = event.startsWith("tap-") || event.startsWith("submit-") ? "click" : "exposure";
      lines.push(`| \`${event}\` | \`${event}\` | ${eventType} | 由事件 detail 决定 | 从 \`ui_to_event\` 绑定生成 |`);
    }
  } else {
...
function generateDataFetching(ui, api, mapping) {
  ...
  if (questions.length > 0) {
    questions.forEach((q, i) => {
      lines.push(`| ${i + 1} | ${mdEscape(q.content)} | PM / 设计 / 后端 | ${mdEscape(q.priority)} |`);
    });
  } else {
    lines.push("| — | 无 | — | — |");
  }
...
function generateSpec(ui, api, mapping, capability) {
  ...
  for (const transition of mapping.state_machine ?? []) {
    ...
    const assertion =
      transition.render_assertion || assertions.get(toState) || "needs_human_input";
    lines.push(
  ...
  if ((!mapping.state_machine || mapping.state_machine.length === 0) && (!ui.states || ui.states.length === 0)) {
    lines.push(
