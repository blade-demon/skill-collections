...
1. <!-- oq:id=oq1 --> `tap-hotspot` 导航到何处？详情页、webview 还是外链？（影响 `detailUrl` 字段。）
2. <!-- oq:id=oq2 --> 整张卡片可点击，还是仅热点行和 CTA 可点击？（影响冒泡模型和 CTA 的 `catchtap`。）
3. <!-- oq:id=oq3 --> 加载态是骨架闪烁、静态占位符，还是隐藏直到加载？
4. <!-- oq:id=oq4 --> 每日热点尚未编排时：完全隐藏卡片，还是显示提示？
5. sparkline 图片加载失败：回退 PNG、空置，还是重试？
6. <!-- oq:id=oq5 --> `error` 态视觉样式待设计签收：行内兜底 + 重试按钮的具体布局是什么？
7. <!-- oq:id=oq6 --> 卡片宽度固定还是响应式？
8. 未登录时 CTA「买一笔」是启用还是先触发登录？
9. <!-- oq:id=oq7 --> `tap-hotspot` 和 `tap-buy` 的分析事件命名？
10. <!-- oq:id=oq8 --> request 封装是否提供 `abort` / `cancel` 能力？重试时是否需要取消前次在途请求？
