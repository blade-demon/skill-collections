...
- 覆盖面增强(P3):① **非 REST 通道**——`03` 第 7 节扩为「消息、异步与非 REST 通道」(MQ 生产/消费、WebSocket/STOMP、SSE、GraphQL、gRPC),`04` 新增「非 REST 通道」清单(不塞进 HTTP 表),`05` 增「异步边界」处理(跨队列不当同步闭环),`07` 让 architecture 图/ backend.md 体现非 REST 通道;`index.json` 新增可选 `channels[]`(schema + validate-docs 同等校验路径/符号/Evidence)。② **前端组件树深度**——`02` 第 4 节扩为「组件目录与组件树」(核心页面组件树、自定义 hooks、状态来源拆分、prop drilling 信号),`07` frontend.md 增「组件构成与状态流」。均为按技术栈存在情况的条件式补充,无则跳过。
- 执行模式与适用范围(P2,纯文档):① **fast 模式**(opt-in)——强模型可连跑多阶段,只在 P5 选链路前与 P8 校验后停;落盘/证据/置信度纪律不变,默认仍逐阶段停下。② **适用范围与多服务降级**——明确主场景为单前端+单后端;多独立后端服务/微服务/网关按「分服务分别跑」或「主服务优先」降级,跨服务调用当「外部系统」处理,不臆测对端。③ **环境预检**(可选,默认关闭)——仅当用户要求时,在证据管线之外实跑 install/build/start 并记录到 `preflight-report.md`,让 onboarding 注明命令「已实测」;P8 仍不执行任何命令。同步修正 `01-discovery.md` 第 5 节「P8 验证可执行性」的过时表述。
