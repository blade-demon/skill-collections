  ...
  images: [
    {
      ...
      signature: { T: 'nav -> title', M: 'card(media + card(title -> meta) -> status)', B: 'action', O: '-', F: '-' },
      notes: { overlay_type: null, float_anchor: null, divider: 'dashed' },
    },
    {
      filename: 'used.png',
      signature: { T: 'nav -> title', M: 'card(media + card(title -> meta) -> status)', B: 'hint', O: '-', F: '-' },
      notes: { overlay_type: null, float_anchor: null, divider: 'dashed' },
    },
  ],
}
...
describe('validateBatchResult', () => {
  ...
  it('rejects when batch id does not match', () => {
    ...
    if (!result.valid) expect(result.errors).toContain('batch id mismatch: expected "batch-2", got "batch-1"')
  })
  ...
  it('rejects invalid slot expression', () => {
    const bad = {
      ...validBatch,
      images: [
        { ...validBatch.images[0]!, signature: { T: 'section(title)', M: '-', B: '-', O: '-', F: '-' } },
        validBatch.images[1]!,
      ],
    }
    const result = validateBatchResult(bad, 'batch-1', ['pending.png', 'used.png'])
  ...
  it('rejects forbidden notes key', () => {
    const bad = {
      ...validBatch,
      images: [
        { ...validBatch.images[0]!, notes: { overlay_type: null, float_anchor: null, bg: 'warm' } as never },
        validBatch.images[1]!,
      ],
    }
    const result = validateBatchResult(bad, 'batch-1', ['pending.png', 'used.png'])
