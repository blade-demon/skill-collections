...
describe('validateCoarseBatchResult', () => {
  ...
  it('rejects unknown role in coarse slot', () => {
    const bad = {
      ...validCoarse,
      images: [
        { ...validCoarse.images[0]!, coarse_signature: { T: ['unknown'], M: ['card'], B: ['action'] } },
  ...
  it('rejects role with operators in coarse slot', () => {
    const bad = {
      ...validCoarse,
      images: [
        { ...validCoarse.images[0]!, coarse_signature: { T: ['nav -> title'], M: ['card'], B: ['action'] } },
        validCoarse.images[1]!,
      ],
    }
    const result = validateCoarseBatchResult(bad as never, 'batch-1', ['a.png', 'b.png'])
  ...
  it('rejects invalid reason string', () => {
    const bad = {
      ...validCoarse,
      images: [
        { ...validCoarse.images[0]!, reason: 'my custom reason' },
        validCoarse.images[1]!,
      ],
    }
    const result = validateCoarseBatchResult(bad as never, 'batch-1', ['a.png', 'b.png'])
