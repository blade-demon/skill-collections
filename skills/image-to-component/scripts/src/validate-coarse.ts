          ...
          if (slot in sig && Array.isArray(sig[slot])) {
            for (const role of sig[slot] as unknown[]) {
              if (typeof role === 'string' && /->|\+|\(|\)/.test(role)) {
                const filename =
                  img.filename && typeof img.filename === 'string' ? img.filename : `image[${i}]`;
                errors.push(
                  `image "${filename}" coarse ${slot}: role "${role}" contains operators — coarse signatures must be flat role arrays only`,
                );
              }
            }
          }
