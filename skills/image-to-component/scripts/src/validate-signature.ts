...
export function validateBatchResult(
  ...
  if (parsed.images.length !== expectedFilenames.length) {
    errors.push(
      `image count mismatch: expected ${expectedFilenames.length}, got ${parsed.images.length}`,
    );
  }
  ...
  for (const img of parsed.images) {
    ...
    if (!expectedFilenames.includes(img.filename)) {
      errors.push(
        `image "${img.filename}": not in expected filenames [${expectedFilenames.join(', ')}]`,
      );
    }
    ...
    if (img.signature.O !== '-' && !img.notes.overlay_type) {
      errors.push(
        `image "${img.filename}": O slot is not "-" but notes.overlay_type is missing or null`,
      );
    }
    if (img.signature.F !== '-' && !img.notes.float_anchor) {
      errors.push(
        `image "${img.filename}": F slot is not "-" but notes.float_anchor is missing or null`,
      );
    }
  }
...
if (process.argv[1] === new URL(import.meta.url).pathname) {
  ...
  const expectedFiles =
    filesIndex >= 0 ? args.slice(filesIndex + 1).filter((a) => !a.startsWith('--')) : [];
  ...
  process.stdin.on('end', () => {
    ...
    try {
      ...
      process.stdout.write(
        JSON.stringify({ valid: false, errors: ['input is not valid JSON'] }) + '\n',
      );
      process.exit(1);
    }
