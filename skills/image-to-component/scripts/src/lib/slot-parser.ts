...
export interface ValidResult { valid: true }
export interface InvalidResult { valid: false; error: string }
...
class Parser {
  ...
  parse(): void {
    ...
    if (this.pos !== this.input.length) {
      throw new ParseError(`Unexpected token at position ${this.pos}: "${this.input.slice(this.pos, this.pos + 15)}"`)
    }
  }
  ...
  private seq(insideRow: boolean): void {
    ...
    while (this.peek(' -> ')) {
      if (insideRow) {
        throw new ParseError(`Bare ' -> ' sequence is not allowed on the right side of '+'. Wrap in a container.`)
      }
      this.consume(' -> ')
      this.row()
    }
  ...
  private atom(insideRow: boolean): void {
    ...
    if (!role) {
      throw new ParseError(`Expected role word at position ${this.pos}, got "${this.input.slice(this.pos, this.pos + 10)}"`)
    }
  ...
  private readRole(): RoleWord | null {
    ...
    if (unknownMatch) {
      throw new ParseError(`Unknown role word '${unknownMatch[0]}'. Allowed: ${ROLE_WORDS.join(', ')}`)
    }
    return null
  }
  ...
  private tryConsume(s: string): boolean {
    if (this.peek(s)) { this.pos += s.length; return true }
    return false
  }
}
