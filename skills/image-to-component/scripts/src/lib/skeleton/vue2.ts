...
export function generateVue2(config: SkeletonConfig): GeneratedFile[] {
  ...
  // Root component
  const rootContent = isCssModules ? genRootCssModules(rootComponent) : genRootBem(rootComponent);
  files.push({ path: `${rootComponent.name}.vue`, content: rootContent });

  // Child components
  for (const child of rootComponent.children) {
    const childContent = isCssModules ? genChildCssModules(child) : genChildBem(child);
