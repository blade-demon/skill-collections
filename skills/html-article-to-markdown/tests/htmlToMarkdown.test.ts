...
test("buildMarkdown converts the target article body to Markdown", async () => {
  ...
  const htmlPath = await writeArticle(root, "<p>Before</p><blockquote><p>Quoted text</p></blockquote>");
...
test("buildMarkdown copies local images with byte-derived extensions", async () => {
  ...
  await writeFile(join(assetRoot, "image"), Buffer.concat([Buffer.from([0x89, 0x50, 0x4e, 0x47]), Buffer.alloc(16)]));
